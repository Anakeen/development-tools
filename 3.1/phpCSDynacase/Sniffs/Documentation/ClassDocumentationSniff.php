 ...
 * @version   CVS: $Id: ClassCommentSniff.php 302088 2010-08-11 01:52:10Z squiz $
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class phpCSDynacase_Sniffs_Documentation_ClassDocumentationSniff implements PHP_CodeSniffer_Sniff
{

    public $neededTag = array(
        'brief',
        'class' 
    );
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        foreach ( $tags as $errorTag ) {
            $data = $errorTag['tag'];
            $place = array_search($data, $this->neededTag);
            if ($place !== false) {
                unset($this->neededTag[$place]);
            }
        }

        foreach ( $this->neededTag as $currentTag ) {
            $error = '@%s tag is needed in class comment';
            $data = array(
                $currentTag
            );
            $phpcsFile->addWarning($error, $stackPtr, 'AllTagAreNeeded', $data);
        }
