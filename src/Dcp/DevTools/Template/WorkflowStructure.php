...
class WorkflowStructure extends Template
{
    public function render($arguments, $outputPath, $force = false)
    {
        ...
        if (!isset($arguments["name"]) || !$this->checkLogicalName($arguments["name"])) {
            throw new Exception("You need to set the name of the workflow with a valid name " .
                $this->logicalNameRegExp);
        }
        if (!isset($arguments["namespace"]) || !$this->checkLogicalName($arguments["namespace"])) {
            throw new Exception("You need to set the namespace of the workflow with a valid name " .
                $this->logicalNameRegExp);
        }
        if (isset($arguments["parent"]) && !$this->checkLogicalName($arguments["parent"])) {
            throw new Exception("You need to set the parent of the workflow with a valid name " .
                $this->logicalNameRegExp);
        }
        ...
        return parent::mainRender("workflow_struct", $arguments, $outputPath, $force);
    }
}
